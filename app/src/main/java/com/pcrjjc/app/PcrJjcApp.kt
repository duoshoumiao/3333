...
class PcrJjcApp : Application(), Configuration.Provider {    
    ...
    private fun createNotificationChannels() {    
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {    
            ...
            // 新增：会战通知渠道  
            val clanBattleChannel = NotificationChannel(  
                CLAN_BATTLE_CHANNEL_ID,  
                "会战通知",  
                NotificationManager.IMPORTANCE_DEFAULT  
            ).apply {  
                description = "会战状态变动通知（击杀清除等）"  
            }  
            ...
            notificationManager.createNotificationChannel(clanBattleChannel)  // 新增  
        }    
    }    
    ...
    companion object {    
        ...
        const val CLAN_BATTLE_CHANNEL_ID = "clan_battle_channel"  // 新增  
    }    
}