...
class RankMonitor(  
    ...
    suspend fun loadCacheFromDb() {  
        if (cache.isEmpty()) {  
            ...
            if (dbCaches.isNotEmpty()) Log.i(TAG, "Loaded ${dbCaches.size} rank cache entries from DB")  
        }  
    }  
    ...
    suspend fun processResult(result: QueryEngine.QueryResult) {  
        ...
        cache[cacheKey] = current  
        rankCacheDao.upsert(RankCache(bind.pcrid, bind.platform, arenaRank, grandArenaRank, lastLoginTime))  
        if (current[0] != previous[0]) handleRankChange(current[0], previous[0], bind, NoticeType.JJC)  
        if (current[1] != previous[1]) handleRankChange(current[1], previous[1], bind, NoticeType.PJJC)  
        if (current[2] != previous[2]) handleRankChange(current[2], previous[2], bind, NoticeType.ONLINE)  
    }  
  
    private suspend fun handleRankChange(new: Int, old: Int, bind: PcrBind, noticeType: NoticeType) {  
        ...
        if (noticeType == NoticeType.ONLINE) {  
            ...
            change = if (new < old) "$prefix$old->$new [▲${old - new}]" else "$prefix$old->$new [▽${new - old}]"  
            val history = JjcHistory(pcrid = bind.pcrid, name = bind.name ?: "", platform = bind.platform, date = timestamp, item = if (isJjc) 0 else 1, before = old, after = new)  
    ...
    suspend fun flushHistories() {  
        ...
        synchronized(pendingHistories) { toInsert = pendingHistories.toList(); pendingHistories.clear() }  
    ...
    fun getCachedRank(pcrid: Long, platform: Int): IntArray? = cache[Pair(pcrid, platform)]  
}